...
public class RenameIT extends LightweightPluginDaemonTest {
  ...
  public void testRenamePropagation() throws Exception {
    ...
    String request =
        Joiner.on("/")
            .join(URL, "a", PROJECTS_API, project.get(), PLUGIN_NAME + "~" + RENAME_ACTION);
  ...
  private RestResponse renameProjectTo(String newName) throws Exception {
    ...
    sender.clear();
    String endPoint =
        "/" + PROJECTS_API + "/" + project.get() + "/" + PLUGIN_NAME + "~" + RENAME_ACTION;
