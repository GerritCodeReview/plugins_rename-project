...
public class RenameIT extends LightweightPluginDaemonTest {
  ...
  public void testRenameViaHttpWithEmptyNewName() throws Exception {
    createChange();
    String newProjectName = "";
    RestResponse r = renameProjectTo(newProjectName, true);
    r.assertBadRequest();
  ...
  public void testRenameReplication() throws Exception {
    ...
    Input input = createInput(NEW_PROJECT_NAME, true);
  ...
  @Test
  @UseLocalDisk
  public void testRenameViaHttpWithNoReplication() throws Exception {
    createChange();
  ...
  private Input createInput(String newName, boolean withReplicationFalse) {
    Input i = new Input();
    i.name = newName;
    i.continueWithRename = true;
    if (withReplicationFalse) {
      i.replication = false;
    }
    return i;
  }
