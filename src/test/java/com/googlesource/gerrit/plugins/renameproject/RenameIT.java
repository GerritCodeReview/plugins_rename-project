...
public class RenameIT extends LightweightPluginDaemonTest {
  ...
  public void testRenameWatchedProject() throws Exception {
    ...
    assertThat(watchedProjects.stream().anyMatch(pwi -> pwi.project.equals(oldProject))).isTrue();

    adminSshSession.exec(PLUGIN_NAME + " " + oldProject + " " + NEW_PROJECT_NAME);
    adminSshSession.assertSuccess();

    watchedProjects = gApi.accounts().self().getWatchedProjects();
    assertThat(watchedProjects.stream().anyMatch(pwi -> pwi.project.equals(NEW_PROJECT_NAME)))
        .isTrue();
    assertThat(watchedProjects.stream().anyMatch(pwi -> pwi.project.equals(oldProject))).isFalse();
  }
}
