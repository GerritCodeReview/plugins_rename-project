...
class HttpClientProvider implements Provider<CloseableHttpClient> {
  ...
  private RequestConfig customRequestConfig() {
    return RequestConfig.custom()
        ...
        .setConnectionRequestTimeout(cfg.connectionTimeout())
        .build();
  }
  ...
  private BasicCredentialsProvider buildCredentials() {
    ...
    log.info("Build creds: " + cfg.user() + "  " + cfg.password());
    creds.setCredentials(
        AuthScope.ANY, new UsernamePasswordCredentials(cfg.user(), cfg.password()));
    return creds;
  }
